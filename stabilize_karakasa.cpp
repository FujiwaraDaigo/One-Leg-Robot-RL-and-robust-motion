...
for(int i=0;i<1;i++){
       ...
       if(temp_cost>sum(statecostkt[i])){ lamda_count++;}
        //judge_CMA_ES=10000;
}
//sample_number=K_max;
...
angular_vel=-(angle[K_number+p_n_2][i][j]-psi[K_number+p_n_2](3*i+j,step_p+1-(step_p%1)))/(step_size);
if(angular_vel>10*u_max){
    angular_vel=10*u_max;
}else if(angular_vel<10*u_min){
angular_vel=10*u_min;
}

real_vel[K_number+p_n_2](3*i+j,step_p)=angular_vel;
    dJointSetHingeParam(Joint_Motor[p_n_2][i][j],  dParamVel, angular_vel);
}else{
     angular_vel=-(angle[K_number+p_n_2][i][j]-psi[K_number+p_n_2](3*i+j,step_p+1-(step_p%1)))/(step_size);
     if(angular_vel>10*u_max){
    angular_vel=10*u_max;
...
void PI2(){
        ...
        int count_p=0;
        double temp1,temp2;
        cost_population_best=100000000000000000;
        for(int i=0;i<K_max;i++){
            if(sum(statecostkt[i])<temp_cost){
                count_p++;
            }
           // cost_judge+=sum(statecostkt[i]);
            if(sum(statecostkt[i])<cost_population_best){
                cost_population_best=sum(statecostkt[i]);
...
///100step待って，足が地面につくの待つ
    if(STEP_p<100){
        ...
        set_colm(statecostkt[K_number+t_num],STEP_p)=(-(temp-sum_read));
...
void create_myutation(){
    printf("myutation\n");
    cout<<(sqrt(cost_population_best*cost_population_best)/(sqrt(temp_cost*temp_cost)+0.0001))<<endl;
    printf("K_max\n");
    cout<<K_max_o<<endl;
    double test;
    test=(sqrt(cost_population_best*cost_population_best)/(sqrt(temp_cost*temp_cost)+0.0001));
   if(test>2){

                                myu_step_d=(exp(-(5*m_count)/sample_total)+exp(-(sqrt((temp_cost-cost_population_best)*(temp_cost-cost_population_best))/sqrt(temp_cost*temp_cost))));
                        ...
                        if(K_max_o<4){
                            jal=10000;
                            max_update=0;
                              myu_step=myu_step*(exp(-(2*m_count)/sample_total)+exp(-(sqrt((temp_cost-cost_population_best)*(temp_cost-cost_population_best))/sqrt(temp_cost*temp_cost))));
...
void update_input(){
  ...
  if(temp_cost>sum(statecostkt[0])||episode==1){
                if(sum(statecostkt[0])>cost_population_best&&episode!=1){
                    temp_cost=cost_population_best;
        ...
        temp_cost=sum(statecostkt[0]);
        old_theta=theta;
                }
        best_theta_count=(epic+1);
        }else{
            if(temp_cost>cost_population_best){
...
void proposed(int trial_count){
    printf("trial_count=%d\n",trial_count);
    while(epic<=rollout){
        if(trial_count>=(max_trial_num)){
            dCloseODE();
        }
...
///end/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    output_goal=fopen("policy_transition.txt","w");
    for(int i=0;i<palallel_p;i++){
...
*/
    ...
    for(int trial_count=0;trial_count<max_trial_num;trial_count++){

        proposed(trial_count);
        set_colm(excel,range((trial_count)*4,(trial_count+1)*4-1))=copy_m;
        for(int i=0;i<num;i++){
            double temp_tra_o;
            temp_tra_o=theta(i,0);
